...
{
	public class VerifyControllerVoice : Controller
	{
        private readonly IConfiguration _configuration;

		public VerifyControllerVoice(IConfiguration configuration)
		{
		...
		public String Number(String number)
		{
			...
			if (value == null)
			{
				int code = helper.SendVerificationCodeCall(DstNumber);
				var key = string.Format("number:{0}:code", number);
				conn.StringSet(key, code, TimeSpan.FromSeconds(60));
			}
			else
			{
				int code = helper.SendVerificationCodePhlo(DstNumber,"call");
				var key = string.Format("number:{0}:code", number);
				conn.StringSet(key, code, TimeSpan.FromSeconds(60));
			}
